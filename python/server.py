...
def main():
    ...
    """
    Self signed certificate & key generated by openssl
    """
    try:
        context.load_cert_chain(certfile="server.crt", keyfile="server.key")
    except Exception as e:
        print(f"[{time.asctime()}] TLS setup error: {e}")
        return
    try:
        ...
        while True:
            ...
            # Wrapping the client socket with TLS
            try:
                tls_socket = context.wrap_socket(client_socket, server_side=True)
            except ssl.SSLError as e:
                print(f"[{time.asctime()}] TLS handshake error: {e}")
                client_socket.close()
                continue
            thread = threading.Thread(target=handle_connection, args=(tls_socket,))
            thread.start()
    except Exception as e:
